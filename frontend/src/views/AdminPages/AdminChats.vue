...
.chats-wrapper { padding: 16px 24px }
.list { display:flex; flex-direction:column; gap:10px }
.conversation { display:grid; grid-template-columns:48px 1fr auto; gap:12px; align-items:center; padding:12px; border-radius:12px; background:#fff; cursor:pointer; border:1px solid #f1f5f9 }
.avatar-placeholder { width:48px; height:48px; background:#eee; border-radius:8px; display:grid; place-items:center; font-weight:700 }
.title { font-weight:600 }
.body { color:#555; font-size:14px }
.meta { display:flex; flex-direction:column; align-items:flex-end; gap:6px }
.unread-count { background:#ef4444; color:#fff; padding:4px 8px; border-radius:10px; font-size:12px }
.time { font-size:12px; color:#9ca3af }
.empty { color:#777; text-align:center; margin-top:40px }
.pagination { margin-top:14px; display:flex; gap:8px; justify-content:center }
.pagination button { padding:6px 10px; border-radius:8px; border:1px solid #e5e7eb; background:#fff }
.pagination button.active { background:#5b3df5; color:#fff }
</style>
